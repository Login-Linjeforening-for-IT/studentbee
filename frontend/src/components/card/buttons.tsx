...
export default function Buttons({ animateAnswer, navigate, flashColor }: ButtonsProps) {
    ...
    return (
        <div className='w-full flex gap-4 mt-2'>
            <button
                className={`${button} ${animateAnswer === 'back' ? 'bg-login-800 border-login-600 scale-95' :
                    'bg-login-900 border-login-800 hover:bg-login-800 text-login-300'}`}
                onClick={() => navigate('back')}
            >
                Back
            </button>
            <button
                className={`${button} ${animateAnswer === 'skip' ? 'bg-login-800 border-login-600 scale-95' :
                    'bg-login-900 border-login-800 hover:bg-login-800 text-login-300'}`}
